...
const MintToken = ({ walletId, tokenId }) => {
  ...
  const mintToken = () => {
    ...
    axios('https://protocol-sandbox.lumx.io/v2/transactions/mints', options)
      .then(response => setMintResponse(response.data))
      .catch(err => console.error(err));
  };

  return (
    <div>
      <button onClick={mintToken} disabled={!walletId || !tokenId}>Mint Token</button>
      {mintResponse && <p>Mint Response: {JSON.stringify(mintResponse)}</p>}
    </div>
  );
};
